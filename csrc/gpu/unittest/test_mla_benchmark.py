...
def mqa_attention(query, p_compressed_kv, compressed_kv, p_key_pe, key_pe, token_num, softmax_scale):
    ...
    p_compressed_kv = p_compressed_kv.reshape((token_num, cache_length, head_dim_v))
    compressed_kv = compressed_kv.reshape((token_num, 1, head_dim_v))
    p_key_pe = p_key_pe.reshape((token_num, cache_length, pe_size))
    key_pe = key_pe.reshape((token_num, 1, pe_size))

    p_k = paddle.concat([p_compressed_kv, p_key_pe], axis=-1).reshape((token_num, cache_length, head_dim_qk))
    k = paddle.concat([compressed_kv, key_pe], axis=-1).reshape((token_num, 1, head_dim_qk))
    k = paddle.concat([p_k, k], axis=1).transpose([0, 2, 1])
    v = paddle.concat([p_compressed_kv, compressed_kv], axis=1).reshape((token_num, cache_length + 1, head_dim_v))

    out = paddle.zeros(shape=[token_num, num_q_head, head_dim_v], dtype=dtype)
